...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".hdf5")
...
class DATA_LOADER_refine(object):
    def __init__(self, opt, net_PFR, netGA, dataloader):
        self.opt = opt
        ...
        self.read_matdataset(opt, self.pre_refine_features(
            net_PFR, netGA, dataloader, opt.device))
        self.index_in_epoch = 0
        self.epochs_completed = 0

    def read_matdataset(self, opt, PreFR_features_dict):
        matcontent = h5py.File(opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".hdf5")
    ...
    def pre_refine_features(self, model, netGA, dataloader, device, extract_batch_size=50):
...
def val_gzsl(test_X, test_label, target_classes,in_package,bias = 0):
    ...
    netGA = in_package['netGA']
    netF = in_package['netF']
    ...
    with torch.no_grad():
        ...
        for i in range(0, ntest, batch_size):
            ...
            inputA, _, _ = netGA(input)
            # inputA = inputA.permute(0, 2, 1)
            # inputA, _, _ = netGA(inputA)
            out_package = netF(inputA)
...
def eval_zs_gzsl(dataloader,netGA, netF,device,bias_seen=0, bias_unseen=0, batch_size=50):
    netGA.eval()
    netF.eval()
    ...
    in_package = {'netGA':netGA, 'netF': netF,'device':device, 'batch_size':batch_size}
